  ...
  if (calibrate == 0) {
    ...
    if (!rtcMem.count) {
      if (weight > (Rodent_Weight + rtcMem.TrapWeight)) {
        ...
        Serial.println("--------------------------------------------------");
        Serial.println();
        ESP.deepSleep(900e6); // Number before 'e' is the duration (in seconds) to be in deep sleep
    ...
    Serial.println("--------------------------------------------------");
    Serial.println();
    ESP.deepSleep(3600e6); // Number before 'e' is the duration (in seconds) to be in deep sleep
    Serial.println("ERROR! Should be sleeping!");
  }
}
